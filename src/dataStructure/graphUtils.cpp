...
std::unordered_map<IDType, std::pair<float, IDType>> Dijkstra(WeightedGraph const &graph, IDType const start, IDType const end)
{
    ...
    while (!to_visit.empty())
    {
        float current_weight = to_visit.top().first;
        IDType current_node = to_visit.top().second;
        to_visit.pop();
        if (current_node == end)
        {
            break;
        ...
        for (DataStructure::WeightedArc const &edge : graph.get_neighbors(current_node))
        {
            float weight = current_weight + edge.weight;

            if (!distances.contains(edge.to) || weight < distances[edge.to].first)
            {
                distances[edge.to] = {weight, current_node};
                to_visit.push({weight, edge.to});
            }
        }
    }
